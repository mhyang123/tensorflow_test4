...
class NaiveBayesClassfier:
    ...
    def word_probabilities(self, counts,total_class0,total_calss1,k):
        #단어의 빈도수를 [단어,p(w/근정),p(w/부정)]형태로 변환
        return[(w,
                (class0+k)/(total_class0 +2 *k),
                (class1+k)/(total_class1 +2 *k))
               for w,(class0,class1)in counts.items]
    def class0_prdbblilty(self,word_probs,doc):
        docwords=doc.split( )#별토코그나이즈 없이 띄어쓰기만 적요해서 분리
        log_proc_if_class0=log_prob_if_class1=0.0#초기화
        for word,prob_if_class0, log_prob_if_class1 in word_probs:
            #만약 리뷰에 word가 나타나면 해당 단어가 나올 log에 확률으 ㄹ더해줌
            if word in  docwords:
    ...
    def train(self,trainfile_path):
        ...
        num_class0=len([1 for -,point in training_set if point >3.5])
    ...
    return self.class0_probability(self.word_)
